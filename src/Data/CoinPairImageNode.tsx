...
export function CoinPairImageNode({coin1,coin2}:{coin1:{img:StaticImageData | string,alt:string},coin2:{img:StaticImageData | string,alt:string}}) {
  return (
    <div className=' w-16 relative h-10'>
        <Image src={coin1.img} alt={coin1.alt} className='w-8 h-8 rounded-full absolute z-10 ' />
        <Image  src={coin2.img} alt={coin2.alt} className='w-8 h-8 rounded-full absolute left-[20px] top-[5px]' />
    </div>
  )
}
